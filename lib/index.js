...
function _generateWitnessSorter({seed}) {
  return (a, b) => {
    ...
    a._hashBuffer = a._hashBuffer || xor(seed, Buffer.from(a.id));
    b._hashBuffer = b._hashBuffer || xor(seed, Buffer.from(b.id));
...
async function _getWitnessPoolWitnesses({ledgerConfiguration, ledgerNode}) {
  ...
  return {
    ...
    secondaryWitnessCandidates: record.secondaryWitnessCandidate
  };
}
